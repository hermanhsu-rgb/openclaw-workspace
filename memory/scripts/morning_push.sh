#!/bin/bash
# morning_push.sh - 晨间推送（直接通过 OpenClaw API）
...
curl -s -X POST "http://localhost:3000/api/messages/send" \
    ...
    -d "{\"channel\":\"wecom\",\"target\":\"heng-kaihsu\",\"content\":\"$(echo "$MESSAGE" | sed 's/\\\\/\\\\\\\\/g; s/"/\\\\"/g' | tr '\n' '|' | sed 's/|/\\n/g')\"}" \
    2>/dev/null

echo "[$(date)] $TITLE 已推送" >> ~/.openclaw/workspace/memory/morning_push.log
